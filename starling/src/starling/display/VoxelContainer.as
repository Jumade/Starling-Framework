...
public class VoxelContainer extends DisplayObjectContainer {
    ...
    private var _worldLight:Vector.<Number>;
    private var _worldLightPosition:Vector3D;

    public function VoxelContainer(p_perspective:Number = 500) {
        touchable = false;
        ...
        _worldLightPosition = new Vector3D(10,10,-20);
        _worldLight = new <Number>[1 , 0, 0,.5];
    ...
    public override function render(support:RenderSupport, p_parentUpdateTransform:Boolean, p_parentUpdateColor:Boolean, p_draw:Boolean):void {
       ...
       var lMatrix:Matrix3D = new Matrix3D()
        if (_xRotation != 0)
            lMatrix.appendRotation(-_xRotation, AXIS_X);
        if (_yRotation != 0)
            lMatrix.appendRotation(-_yRotation, AXIS_Y);
        if (_zRotation != 0)
            lMatrix.appendRotation(-_zRotation, AXIS_Z);

        var l:Vector3D = lMatrix.transformVector(_worldLightPosition)
        l.normalize();
        l.scaleBy(.5)
        _worldLight[0] = l.x;
        _worldLight[1] = l.y;
        ...
        Starling.context.setProgramConstantsFromVector(Context3DProgramType.VERTEX,4,_worldLight);

        Starling.context.setDepthTest(true, Context3DCompareMode.LESS_EQUAL);
        ...
        Starling.context.setDepthTest(false, Context3DCompareMode.ALWAYS);
